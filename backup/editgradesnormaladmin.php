 ...
 <?php
        $id = mysqli_real_escape_string($conn, $_GET['id']);
         if(isset($_POST["submit"])){
            ...
            $sql = "UPDATE tbl_userinfo, tbl_useraddress, tbl_school, tbl_grades SET tbl_useraddress.barangay = '$brgy', tbl_school.SchoolIntended = '$last_school', tbl_userinfo.batch = '$batch', tbl_school.Course = '$course', tbl_userinfo.remarks = '$remarks',  tbl_userinfo.assessment = '$assessment', tbl_school.SchoolYear = '$schoolYr', tbl_userinfo.Category = '$Category', tbl_grades.GWA = '$gwaNow', tbl_grades.f_semester_grade = '$f_semester_grade', tbl_grades.s_semester_grade = '$s_semester_grade', tbl_grades.yrlvl = '$yrlvl', tbl_grades.semester = '$semester'  WHERE tbl_userinfo.id = tbl_useraddress.user_id AND tbl_userinfo.id = tbl_school.user_id AND tbl_userinfo.id = tbl_grades.user_id AND tbl_userinfo.id = '$id' AND tbl_grades.id = (SELECT MAX(tbl_grades.id) FROM (select * from tbl_grades) as tbl_grades)";

            // $sql1 = "INSERT INTO tbl_grades(user_id,GWA, f_semester_grade, s_semester_grade, yrLvl, semester) VALUES ('$id','$gwaNow', '$f_semester_grade', '$s_semester_grade', '$yrlvl', '$semester')";
            if(mysqli_query($conn, $sql))
              {
              echo '<script language="javascript">';
              echo 'alert("USER SUCCESSFULLY UPDATED!")';
              echo '</script>';
              header("location: manageapplicantnormaladmin.php");
              }
            else{
      ...
      if ($result->num_rows > 0) {
          ...
          while ($row = $result->fetch_assoc()) {
              echo "<tr>";
                ...
                echo "<td><input type='yrLvl' value='".$row['yrLvl']."' name='yrLvl' class='form-control input-lg' placeholder='".$row['yrLvl']."'></td>";
                ...
                echo "<td><select name='assessment'><option name ='assessment' value='0'>Not For Renewal</option>
                                  <option name ='assessment' value='1'>For Renewal</option>
                                  </select></td>";
                // echo "<td><input type='assessment' name='assessment' value='".$row['assessment']."' class='form-control input-lg' placeholder='".$row['assessment']."'></td>";
                echo "<td><button class='btn btn-success btn-lg text-white' type='Submit' value ='Submit' name = 'submit'> Update </button></td>";
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            ...
            <a class="btn btn-primary" href="editgradesnormaladmin.php">Back</a>
          </div>
        </div>
      </div>
    </div>
