...
if(($_SESSION['studenttype']!="collegerecipient")||($_SESSION['pages']!="renewnotif-collegerecipient")){
 $_SESSION['studenttype']="collegerecipient";
  $_SESSION['search']="ALL";
  $_SESSION['pages']="renewnotif-collegerecipient";
  $_SESSION['whatsearch']="";
  $_SESSION['order']="ASC";
  $_SESSION['limit']=10;
  $_SESSION['offset']=1;
  ...
  <head>
    ...
    <script>
      ...
      function gotoExisting(id){
        window.open("searchExistingRenew.php?id="+id,"_self");
      }
            ...
            <div class="row page-title-header">
              <div class="col-12">
                <div class="page-header">
                  <h4 class="page-title">Renewal Notification >  College Financial Assistance </h4>
 ...
 $status1="OPEN";
    ...
    $sqla = "SELECT * FROM tbl_renew_year WHERE status='$status1' OR status='$status2'";
     ...
     $academic_id=$rowa['id'];
                      ...
                      $sql = "SELECT tbl_admin.id as adminid,tbl_renew.id,tbl_renew.academic_year,tbl_renew.application_no,tbl_renew.school_id,tbl_renew.registration_form
                        FROM tbl_renew 
                        INNER JOIN tbl_admin 
                        ON tbl_renew.academic_year=tbl_admin.academic_year AND tbl_renew.application_no=tbl_admin.application_no WHERE (tbl_renew.school_id='$stats' OR tbl_renew.registration_form='$stats') AND tbl_admin.usertype='$usertype' AND tbl_admin.typescholar='$scholar' AND tbl_admin.status='$status' AND tbl_renew.academic_id='$academic_id' ORDER BY tbl_renew.id ASC LIMIT ".$limit." OFFSET ".$off;
                        ;
                       ...
                       $sql1 = "SELECT COUNT(tbl_renew.id) as total 
                        ...
                        WHERE (tbl_renew.school_id='$stats' OR tbl_renew.registration_form='$stats') AND tbl_admin.usertype='$usertype' AND tbl_admin.typescholar='$scholar' AND tbl_admin.status='$status' AND tbl_renew.academic_id='$academic_id'";
              ...
              <div class="col-md-12 grid-margin">
                <div class="card">
                  <div class="card-body">
                    <div class="row">
                        <h4 class="card-title"> List of College Financial Assistance </h4>
                    <table class="table">
                      <thead>
                        <tr>
                          <th style="width:5%;">#</th>
                          <th style="width:15%;">Name</th>
                           <th style="width:15%;">School</th>
                            <th style="width:15%;">School Type</th>
                              ...
                              if($row['registration_form']==2){
                                ...
                                $count++;
                                 echo '<tr>';
                                  ...
                                  echo '</td>';
                                   echo '<td style="width:15%;">';
                                  echo $rowg['schooltype'];
                                  echo '</td>';
                                  echo '<td style="width:60%;">';
                                  echo $remark;
                                  echo '</td>';
                                  echo '<td style="width:15%;"><center>';
                                  echo '<form action="searchExistingRenew.php" method="POST">';
                                  ...
                                  echo '" name="ids">';
                                     echo '<button type="submit" id="';
                                          ...
                                          echo '" type="button" class="btn btn-icons btn-rounded btn-primary">
                                            <i class="fa fa-eye" data-toggle="tooltip" data-placement="top" title="Click to search existing college financial assistance"></i>';
                                             echo '</button>';
                                  echo '</form>';
                                  echo '</center></td>';
                                  echo '</tr>';
                      ...
                      <?php
                        if($_SESSION['page']<=1){
                          echo '<button class="btn btn-warning" disabled>Previous</button>';
                           ...
                           echo '<a href="back_renew.php" class="btn btn-warning" >Previous</a>';
                        }
                      ?>
                       <?php echo ($off+1)."-".$count.'/'.$total?>
                        ...
                        if($count>=$total){
                          echo '<button class="btn btn-primary" disabled>Next</button>';
                           ...
                           echo '<a href="next_renew.php" class="btn btn-primary">Next</a>';
                        }
                      ?>
